...
namespace Codemasters.Telemetry.F12019
{
        ...
        IObservable<SessionData>
    {
        ...
        protected override void NotifyData(Memory<byte> data)
        {
            base.NotifyData(data);

            try
            {
                var packetHeader = PacketHeaderHelper.ReadHeader(data.Span);

                var input = data.Slice(24).Span;

                switch (packetHeader.PacketId)
                {
                    case F1PacketType.CarSetups: NotifyData(_carSetupDataReader.Read(input, packetHeader)); break;
                    case F1PacketType.CarStatus: NotifyData(_carStatusDataReader.Read(input, packetHeader)); break;
                    case F1PacketType.CarTelemetry: NotifyData(_carTelemetryDataReader.Read(input, packetHeader)); break;
                    case F1PacketType.Event: NotifyData(_eventDataReader.Read(input, packetHeader)); break;
                    case F1PacketType.LapData: NotifyData(_lapDataReader.Read(input, packetHeader)); break;
                    case F1PacketType.Motion: NotifyData(_motionDataReader.Read(input, packetHeader)); break;
                    case F1PacketType.Participants: NotifyData(_participantsDataReader.Read(input, packetHeader)); break;
                    case F1PacketType.Session: NotifyData(_sessionDataReader.Read(input, packetHeader)); break;
                    default:
